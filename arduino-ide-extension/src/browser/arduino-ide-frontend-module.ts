...
import { HomeWidgetContribution } from './home/home-widget-contribution';
import { BoardImgWidget } from './boardImg/boardImg-widget';
import { BoardImgWidgetContribution } from './boardImg/boardImg-widget-contribution';
import {
  ApplicationShell,
  bindToolbarApplicationShell,
  DockPanelRenderer,
} from './theia/core/application-shell';
import { MySearchInWorkspaceWidget } from './theia/search-in-workspace/search-in-workspace-widget';
import { SearchInWorkspaceWidget } from '@theia/search-in-workspace/lib/browser/search-in-workspace-widget';
import { OutputWidget } from '@theia/output/lib/browser/output-widget';
import { MyOutputWidget } from './theia/out-put/out-put-widget';
import { KeymapsFrontendWidget } from './theia/keymaps/keymaps-frontend-widget';
import { KeybindingWidget } from '@theia/keymaps/lib/browser/keybindings-widget';
import { MyNotificationsContribution } from './theia/messages/notifications-contribution';
import { NotificationsContribution } from '@theia/messages/lib/browser/notifications-contribution';
import { MyFileSystemWatcherErrorHandler } from './theia/filesystem/filesystem-watcher-error-handler';
import { FileSystemWatcherErrorHandler } from '@theia/filesystem/lib/browser/filesystem-watcher-error-handler';
import { MyLibraryListWidget } from './library/my-library-list-widget';
import { MyLibraryListWidgetFrontendContribution } from './library/my-library-widget-frontend-contribution';
import {
  LibrarysDialog,
  LibrarysDialogProps,
} from './dialogs/librarys/librarys-dialog';
import { OpenLibrarys } from './dialogs/librarys/open-librarys';
import {
  KeymapsDialog,
  KeymapsDialogProps,
} from './dialogs/Keymaps/keymaps-dialog';
...
export default new ContainerModule((bind, unbind, isBound, rebind) => {
  ...
  bind(CommandContribution).to(BoardImgWidgetContribution);
  bind(CommandContribution).to(HomeWidgetContribution);
  ...
  rebind(TheiaKeymapsFrontendContribution)
    ...
    .inSingletonScope();
  rebind(KeybindingWidget).to(KeymapsFrontendWidget).inSingletonScope();
  ...
  Contribution.configure(bind, OpenLibrarys);
  Contribution.configure(bind, OpenKeymaps);
  ...
  bind(LibrarysDialog).toSelf().inSingletonScope();
  bind(LibrarysDialogProps).toConstantValue({
    title: '从本地安装库',
  });

  bind(KeymapsDialog).toSelf().inSingletonScope();
  bind(KeymapsDialogProps).toConstantValue({
    title: '键盘快捷方式参考',
  });

  bind(NotificationManager).toSelf().inSingletonScope();
  rebind(TheiaNotificationManager).toService(NotificationManager);
  bind(MyNotificationsContribution).toSelf().inSingletonScope();
  rebind(NotificationsContribution).toService(MyNotificationsContribution);
  bind(NotificationsRenderer).toSelf().inSingletonScope();
  rebind(TheiaNotificationsRenderer).toService(NotificationsRenderer);

  //====================
  // bind(YourCustomWidget).toSelf();
  // bind(YourCustomWidgetContribution).toSelf();
  // bind(FrontendApplicationContribution).toService(YourCustomWidgetContribution);
  // bind(WidgetFactory).toDynamicValue(({ container }) => ({
  //   id: 'your-custom-widget-id',d
  //   createWidget: () => container.get(YourCustomWidget),
  // }));

  //UI for the BoardImg
  bind(BoardImgWidget).toSelf();
  bind(BoardImgWidgetContribution).toSelf();
  bind(FrontendApplicationContribution).toService(BoardImgWidgetContribution);
  bind(WidgetFactory).toDynamicValue(({ container }) => ({
    id: 'lingzhi-boardImg-widget',
    createWidget: () => container.get(BoardImgWidget),
  }));

  //UI for the library
  bind(MyLibraryListWidget).toSelf();
  bind(MyLibraryListWidgetFrontendContribution).toSelf();
  bind(WidgetFactory).toDynamicValue(({ container }) => ({
    id: 'lingzhi-library-widget',
    createWidget: () => container.get(MyLibraryListWidget),
  }));

  //UI for the Home
  bind(HomeWidget).toSelf();
  bind(HomeWidgetContribution).toSelf();
  // bindViewContribution(bind, HomeWidgetContribution);
  bind(FrontendApplicationContribution).toService(HomeWidgetContribution);
  bind(WidgetFactory).toDynamicValue(({ container }) => ({
    id: 'lingzhi-home-widget',
    createWidget: () => container.get(HomeWidget),
  }));
  ...
  bind(MyFileSystemWatcherErrorHandler).toSelf().inSingletonScope();
  rebind(FileSystemWatcherErrorHandler).toService(
    MyFileSystemWatcherErrorHandler
  );
  ...
  rebind(OutputWidget).to(MyOutputWidget);
