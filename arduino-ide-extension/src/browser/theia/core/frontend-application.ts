...
export class FrontendApplication extends TheiaFrontendApplication {
  ...
  override async start(): Promise<void> {
    ...
    await this.measure(
      'revealShell',
      () => this.revealShell(host),
      'Replace loading indicator with ready workbench UI (animation)',
      false
    );
    this.registerEventListeners();
    this.stateService.state = 'ready';

    startup.then((idToken) =>
      this.backendStopwatch.stop(idToken, 'Frontend application start', [])
    );
  }
