...
import { ResponseService, Sketch, SketchesError } from '../../common/protocol';
import {
  Command,
  CommandRegistry,
  SketchContribution,
  ...
  private async openSketchFiles(
    ...
    try {
      ...
      for (const uri of [mainFileUri, ...rootFolderFileUris]) {
        await this.ensureOpened(mainFileUri);
      }
      ...
      if (mainFileUri.endsWith('.pde')) {
        const message = nls.localize(
          ...
          "'{0}'仍然使用旧的'.pde'的格式。是否要切换到新的'.ino'的扩展?",
          sketch.name
        );
        const yes = '是';
...
export namespace OpenSketchFiles {
  export namespace Commands {
    export const OPEN_SKETCH_FILES: Command = {
      id: 'lingzhi-open-sketch-files',
    };
    export const ENSURE_OPENED: Command = {
      id: 'lingzhi-ensure-opened',
    };
  }
}
