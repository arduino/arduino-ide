      ...
      if (selectedBoard) {
        ...
        if (fqbn) {
          ...
          if (configOptions.length) {
            ...
            for (let { label, option, values } of configOptions.sort(
              ConfigOption.LABEL_COMPARATOR
            )) {
              label = label.replace('Core Debug Level', '核心调试级');
              label = label.replace('CPU Frequency', '处理器外频');
              label = label.replace('Flash Frequency', '闪烁频率');
              label = label.replace('Flash Mode', '闪烁模式');
              label = label.replace('Flash Size', 'Flash大小');
              label = label.replace('Partition Scheme', '分区方案');
              label = label.replace('Upload Speed', '上传速度');
              label = label.replace('Debug Level', '调试级别');
              label = label.replace('Debug port', '调试端口');
              label = label.replace('Erase Flash', '擦除闪存');
              label = label.replace('Exceptions', '异常处理');
              label = label.replace('lwIP Variant', 'lwIP变体');
