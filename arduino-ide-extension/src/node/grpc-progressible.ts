...
export namespace ExecuteWithProgress {
  ...
  export function createDataCallback<R extends ProgressResponse>({
    ...
    return (response: R) => {
      ...
      if (task) {
        let message = task.getName() || task.getMessage();
        ...
        if (message) {
          ...
          message = message.includes('Downloading packages')
            ? '下载包'
            : message;
          message = message.includes('Configuring tool.')
            ? '配置工具'
            : message;
          message = message.includes('Configuring platform.')
            ? '配置平台'
            : message;
          message = message.includes('Installing')
            ? message.replace('Installing', '安装')
            : message;
          message = message.includes('Installed')
            ? message.replace('Installed', '安装')
            : message;
          message = message.includes('Downloading')
            ? message.replace('Downloading', '下载')
            : message;
          message = message.includes('Installing platform')
            ? message.replace('Installing platform', '安装平台')
            : message;
          message = message.includes('Platform')
            ? message.replace('Platform', '平台')
            : message;
          message = message.includes('Already installed')
            ? message.replace('Already installed', '已经安装')
            : message;
          message = message.includes('installed')
            ? message.replace('installed', '已安装')
            : message;
        ...
        if (phase instanceof DownloadProgressStart) {
          ...
          message = message.includes('Downloading index')
            ? message.replace('Downloading index:', '下载索引')
            : message;
          url = phase.getUrl();
          // responseService.appendToOutput?.({ chunk: `${message}\n` });
