...
export class CoreClientProvider extends GrpcClientProvider<CoreClientProvider.Client> {
    ...
    protected async createClient(port: string | number): Promise<CoreClientProvider.Client> {
        ...
        // @ts-ignore
        const ArduinoCoreClient = grpc.makeClientConstructor(commandsGrpcPb['cc.arduino.cli.commands.v1.ArduinoCoreService'], 'ArduinoCoreServiceService') as any;
        const client = new ArduinoCoreClient(`localhost:${port}`, grpc.credentials.createInsecure(), this.channelOptions) as ArduinoCoreServiceClient;
