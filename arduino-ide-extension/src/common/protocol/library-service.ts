...
export interface LibraryService
  ...
  libraryData(value: string): Promise<LibInfo[]>;
  getBoardLibraries(): Promise<string[]>;
  compareVersions(versionA: string, versionB: string): Promise<number>;
  installLibrary(path: string, model: string): Promise<[string, boolean]>;
  manualData(value: string, libraryName: string): Promise<string>;
  installationLibrary(
    url: string,
    savePath: string,
    extractDir: string
  ): Promise<boolean>;
  netWorkingGetLibraryData(value: string): Promise<LibInfo[]>;
