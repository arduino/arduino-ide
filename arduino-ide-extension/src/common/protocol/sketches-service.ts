...
export namespace Sketch {
  ...
  export function reservedFilename(name: string): string {
    return `‘${name}’是一个保留的文件名。`;
  }
  // (non-API) exported for the tests
  export const noTrailingPeriod = '文件名不能以点结尾';
  // (non-API) exported for the tests
  export const invalidSketchFolderNameMessage =
    '名称必须以字母、数字或下划线开头，后面跟着字母、数字、“-”、“。”和“_”。最大长度为63个字符。';
  const invalidCloudSketchFolderNameMessage =
    '名称必须以字母、数字或“_”开头，后跟字母、数字、“-”、“。”和“_”。最大长度为36个字符。';
  /**
  ...
  export function toValidSketchFolderName(
    ...
    if (appendTimestampSuffix) {
      return `${validName.slice(0, 63 - timestampSuffixLength)}${typeof appendTimestampSuffix === 'boolean'
          ? timestampSuffix()
