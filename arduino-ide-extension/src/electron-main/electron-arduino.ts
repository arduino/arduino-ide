...
export class ElectronArduino implements ElectronMainApplicationContribution {
  onStart(app: TheiaElectronMainApplication): void {
    ...
    ipcMain.on(LINGZHI_MINI, (event) => {
      const window = BrowserWindow.fromWebContents(event.sender);
      if (window) {
        window.minimize();
      }
    });
    ipcMain.on(LINGZHI_MAX, (event) => {
      const window = BrowserWindow.fromWebContents(event.sender);
      if (window) {
...
export namespace ElectronArduinoRenderer {
  ...
  // 导出一个函数，用于在WebContents上添加一个监听器，当接收到CHANNEL_REQUEST_RELOAD通道的消息时，调用handler函数
  export function onRequestReload(
    ...
    // 调用createWindowListener函数，创建一个监听器，并返回一个Disposable对象
